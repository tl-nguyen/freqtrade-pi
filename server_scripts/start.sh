...
if [ -z "$STRATEGY_CONTAINER_NAME" ] || [ -z "$STRATEGY_CLASS" ]; then
  ...
  if [ ! -f ~/freqtrade/strategies/$STRATEGY_CONTAINER_NAME.json ] || [ ! -f ~/freqtrade/strategies/$STRATEGY_CONTAINER_NAME.py ]; then
    echo "No $STRATEGY_CONTAINER_NAME exists. You have to run './fp.sh update -s $STRATEGY_CONTAINER_NAME' first"
  else
    ...
    echo "!Press Ctrl+C to stop seeing the logs (this won't stop your strategy!)"
    sudo docker logs -f $STRATEGY_CONTAINER_NAME
  fi
fi